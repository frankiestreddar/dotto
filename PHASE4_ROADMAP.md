...
notification pushed end-to-end) against a fresh production build. CI's e2e job itself is not yet
confirmed green in GitHub Actions — blocked on the user adding the 4 repo secrets (see above); every
other CI step (`lint`/`typecheck`/`format:check`/`test`/`build`) needs no further action.
