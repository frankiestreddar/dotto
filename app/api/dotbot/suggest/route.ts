...
// DOTBOT_SUGGEST_SCHEMA (still defined in lib/dotbot.ts, deliberately left untouched) isn't
// imported here anymore — see the equivalent note in orchestrate/route.ts: Groq has no
...
export async function POST(request: NextRequest) {
  ...
  try {
    const completion = await groq.chat.completions.create({
      ...
      // "none" — see lib/groq.ts. Doubly important here: this fires on nearly every keystroke,
