// Phase 4.1 cluster revisit port of public/dotto/search-panel-history.js (file #10 of 11) —
...
//
// Zero appState dependency (just static DOM elements + localStorage) — same zero-poll,
// DOM-existence-guard-only shape wireThemeToggle/wireSidebarModeToggle already established,
// called once from HamburgerMenu.jsx's own mount useEffect, same markup fragment
// (hamburger-stack.html) those two already reach into.

import { escapeHtml } from "./textUtils";
import { rowActionsHTML } from "./outlineTree";
...
function loadHistory(): string[] {
  try {
    ...
    return []; // private browsing, storage disabled, corrupt JSON, etc.
  }
}

export function wireSearchPanelHistory(): () => void {
  const searchInput = document.getElementById("search-panel-search") as HTMLInputElement | null;
  ...
  function saveHistory(): void {
    try {
      ...
      // private browsing, storage disabled, etc. — history just won't persist across reloads
    }
  }
  ...
  // search.png (not query.png) is this list's own icon, unchanged from before this port.
  function renderHistory(): void {
    ...
    `,
          ...
          .join("")
      : '<div class="outline-empty">No searches yet.</div>';
    historyList!.querySelectorAll(".outline-item").forEach((row) => {
      row.addEventListener("click", () => {
        searchInput!.value = (row as HTMLElement).dataset.query || "";
        searchInput!.focus();
      });
    });
  }
  ...
  function addToHistory(query: string): void {
    ...
    history = [trimmed, ...history.filter((q) => q.toLowerCase() !== trimmed.toLowerCase())].slice(
      0,
      MAX_HISTORY,
    );
    saveHistory();
