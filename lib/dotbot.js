...
- "recommendedSearches": REQUIRED exactly when "canHelp" is false — exactly 3 short, concrete, UNRELATED linguistics tasks (one grammar topic, one word lookup, one translation task).
...
const ALIGNMENT_SCHEMA = {
  type: Type.ARRAY,
  nullable: true,
  description: "One-word-to-one-word (or one-morpheme-to-one-morpheme for agglutinative languages) correspondences between this sentence's \"text\" and \"translation\", so the app can highlight matching parts with the same color. Never group multiple independent words from one side into a single pair just because they translate as a phrase (e.g. Spanish \"en la esquina\"/\"in the corner\" is 3 pairs, not 1) — only combine words when a single word/morpheme genuinely has no isolable individual counterpart on the other side. sourcePhrase/targetPhrase MUST be exact, verbatim substrings of \"text\"/\"translation\" respectively.",
  items: {
    type: Type.OBJECT,
    required: ["sourcePhrase", "targetPhrase"],
    properties: {
      sourcePhrase: { type: Type.STRING, description: "An exact substring of this sentence's \"text\"." },
      targetPhrase: { type: Type.STRING, description: "An exact substring of this sentence's \"translation\"." },
    },
  },
};
...
export const DOTBOT_ORCHESTRATE_SCHEMA = {
  ...
  properties: {
    ...
    canHelp: { type: Type.BOOLEAN, description: "REQUIRED. false only in the off-topic/casual-chat apologize-and-redirect cases — true otherwise." },
    recommendedSearches: {
      type: Type.ARRAY,
      nullable: true,
      items: { type: Type.STRING },
      description: "Exactly 3 short, concrete linguistics tasks (grammar, word lookup, translation) — REQUIRED when canHelp is false, omitted otherwise.",
    },
    ...
    dictionary: {
      type: Type.ARRAY,
      nullable: true,
      description: "1-5 entries, most-common-sense-first. Include ONLY for a word/phrase meaning question.",
      items: {
        type: Type.OBJECT,
        required: ["word", "language", "grammarTags", "definition", "ipa"],
        properties: {
          word: { type: Type.STRING, description: "The headword in its ORIGINAL script (Chinese characters, Cyrillic, Arabic script, Japanese kanji/kana, etc — never transliterated here). Never a sentence or explanation." },
          transliteration: { type: Type.STRING, nullable: true, description: "Latin-script transliteration (pinyin, romaji, etc) ONLY when \"word\" isn't already Latin script. Omit for already-Latin words." },
          ipa: { type: Type.STRING, description: "REQUIRED. IPA phonetic transcription, no surrounding slashes, e.g. \"tɪr\"." },
          language: { type: Type.STRING, description: "BCP-47 code for text-to-speech, e.g. \"en-US\", \"es-ES\", \"ja-JP\", \"zh-CN\"." },
          grammarTags: {
            type: Type.ARRAY,
            items: { type: Type.STRING },
          ...
          definition: { type: Type.STRING, description: "REQUIRED, never empty. HARD RULE: written ENTIRELY in the SAME language as \"word\" itself — NEVER in English, NEVER translated, even when \"word\" is a non-English word being looked up from an English-language query. Short, well under 15 words, one sense only." },
        },
      },
    },
    ...
    translation: {
      ...
      description: "Fill ONLY for a direct translation-style query. Renders as its own panel above the dictionary panel.",
      properties: {
        sourceWord: { type: Type.STRING, description: "The original word/phrase, in its own script." },
        sourceLanguage: { type: Type.STRING, description: "Short, human-readable language name for display (NOT a BCP-47 code), e.g. \"English\"." },
        targetWord: { type: Type.STRING, description: "The translated word/phrase, in its own script." },
        targetLanguage: { type: Type.STRING, description: "Short, human-readable language name for display, e.g. \"Spanish\"." },
      },
    },
        ...
        sentences: {
          ...
          items: {
            ...
            properties: {
              ...
              translation: { type: Type.STRING, description: "English translation (repeat verbatim if already English)." },
              romanization: { type: Type.STRING, nullable: true, description: "Latin-script transliteration of \"text\" (pinyin, romaji, etc) ONLY when \"text\" isn't already Latin script. Omit for already-Latin sentences." },
              alignment: ALIGNMENT_SCHEMA,
            },
          },
        },
        language: { type: Type.STRING, nullable: true, description: "BCP-47 code for text-to-speech playback of these sentences, e.g. \"en-US\", \"es-ES\", \"ja-JP\"." },
      },
    },
    dotbotText: { type: Type.STRING, description: "REQUIRED. Short plain-text answer, 1-4 sentences. Always present, even when dictionary/examples alone are sufficient — keep it short and complementary in that case. For a genuine grammar/explanation question, this stays a short lead-in and the in-depth content goes in \"answerBlocks\" instead." },
    ...
    answerBlocks: {
      type: Type.ARRAY,
      nullable: true,
      description: "An in-depth, well-organized explanation as an ordered sequence of blocks — fill ONLY for a genuine grammar/explanation question, never for a simple lookup.",
      items: {
        type: Type.OBJECT,
        required: ["type"],
        properties: {
          type: { type: Type.STRING, description: "\"text\" for a prose paragraph, or \"example\" for a highlighted example-sentence card woven into the explanation at the point it's relevant." },
          content: { type: Type.STRING, nullable: true, description: "REQUIRED when type is \"text\": one paragraph of plain prose, no markdown/HTML." },
          text: { type: Type.STRING, nullable: true, description: "REQUIRED when type is \"example\": the example sentence." },
          translation: { type: Type.STRING, nullable: true, description: "REQUIRED when type is \"example\": English translation (repeat verbatim if already English)." },
          romanization: { type: Type.STRING, nullable: true, description: "Only when type is \"example\" and \"text\" isn't already Latin script." },
          alignment: ALIGNMENT_SCHEMA,
        },
      },
    },
    ...
    sourceAction: {
      ...
      nullable: true,
      required: ["type", "rows"],
      description: "Fill ONLY when the user wants generated tabular content written into a source — either adding rows to an attached source or creating a brand new one from scratch. Omit entirely otherwise.",
      properties: {
        type: { type: Type.STRING, description: "\"add_rows\" to append generated rows to an attached source (see targetIndex), or \"create_source\" to build a brand new standalone source card." },
        targetIndex: { type: Type.INTEGER, nullable: true, description: "REQUIRED when type is \"add_rows\": the number of the attached source (matching \"Sources attached to this query\") to add the rows to." },
        title: { type: Type.STRING, nullable: true, description: "A short title for the new source. Only used when type is \"create_source\"." },
        columns: {
          type: Type.ARRAY,
          nullable: true,
          items: { type: Type.STRING },
          description: "Column header names, left-to-right, e.g. [\"Word\",\"Translation\"] or [\"Japanese\",\"English\",\"Notes\"]. A source is never limited to 2 columns — use as many as make sense for the content, up to 10. When adding rows to an attached source that ALREADY has named columns, omit this — never rename or add to an existing source's columns.",
        },
        rows: {
          type: Type.ARRAY,
          ...
          items: {
            ...
            required: ["cells"],
            properties: {
              cells: { type: Type.ARRAY, items: { type: Type.STRING }, description: "One cell of text per column, in the SAME order/count as \"columns\"." },
            },
          },
        },
      },
...
export const DOTBOT_SUGGEST_SCHEMA = {
  ...
  properties: {
    suggestions: { type: Type.ARRAY, items: { type: Type.STRING }, description: "Up to 4 short full-sentence completions." },
  },
};
...
export async function peekGenerationCredits(supabase, userId, amount) {
  ...
  const remaining = data.generation_credits_reset_at < monthAgo ? 100 : data.generation_credits_remaining;
  return remaining >= amount;
}
