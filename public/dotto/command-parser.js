...
// used both to drive the live suggestions panel (command-palette.js) and, on Enter, to resolve
// and execute a complete one (command-target-lookup.js/command-verbs.js). Returns null for
// anything that isn't shaped like a command at all, so callers can fall back to normal search.
//
...
function parseCommandInput(value) {
    ...
    return { stage: 'target', kind, targetRaw, verb: vm[1].toLowerCase(), arg: null }; // 'place' | 'copy'
}

export { KINDS, parseCommandInput };
